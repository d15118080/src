...
namespace Raycity.File
{
    ...
    public partial class JmdArchive : IJmdArchive<JmdFile, JmdFolder>
    {
            ...
            while(procssQueue.Count > 0)
            {
                ...
                using(MemoryStream memStream = new MemoryStream(folderData))
                {
                    ...
                    for(int i = 0; i < fileCount; i++)
                    {
                        ...
                        Debug.WriteLine($"filename: {fileName}, extInt: {extInt}, fileProperty: {fileProperty}, dataIndex: {dataIndex}, fileSize: {fileSize}, fileExtension: {fileExtension}");
        ...
        public void AddNewFile(JmdFolder folder, string filepath)
        {
            string fileName = Path.GetFileNameWithoutExtension(filepath);
            string fileExtension = Path.GetExtension(filepath);

            var key = JmdKey.GetJmdKey(filepath);

            FileInfo fileInfo = new FileInfo(filepath);
            int fileSizeInBytes = (int)fileInfo.Length;

            var newFile = new JmdFile();
            int fileProperty = (int)JmdFileProperty.Encrypted;
            newFile.Name = $"{fileName}{fileExtension}";
            newFile.FileEncryptionProperty = (JmdFileProperty)fileProperty;
            newFile.IsNew = true;
            newFile.FullPath = filepath;

            uint extInt = newFile.getExtNum();

            uint dataIndex = newFile.getDataIndex(folder.getFolderDataIndex());
            int fileSize = fileSizeInBytes;
            uint fileKey = JmdKey.GetFileKey(key, fileName, extInt);

            JmdFileHandler fileHandler = new JmdFileHandler(this, (JmdFileProperty)fileProperty, dataIndex, fileSize, fileKey);
            JmdDataSource bufferedDataSource = new JmdDataSource(fileHandler);
            newFile.DataSource = bufferedDataSource;

            _fileHandlers[dataIndex] = fileHandler;
            //_fileHandlers.Add(dataIndex, fileHandler);
